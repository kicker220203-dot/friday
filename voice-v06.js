...
let v06VoiceSettings = (() => {
  try {
    ...
    return { enabled: true, backendUrl: '' };
  }
})();
...
window.speak = async function(text, opts={}) {
  ...
  if (!v06VoiceSettings.enabled || !backend) {
  ...
  try {
    ...
    const response = await fetch(`${backend}/api/tts`, {
      ...
      body: JSON.stringify({ text, persona: persona === 'tuesday' ? 'tuesday' : 'friday' }),
...
async function v06CheckBackend(showResult=true) {
  ...
  try {
    ...
    if (showResult) {
      maybeStatus(ok ? 'Нейроголос подключён.' : 'Сервер найден, но ElevenLabs ещё не настроен.', true, 3000);
    }
    return ok;
  } catch {
    v06BackendState = 'offline';
...
function v06VoiceStateLabel() {
  ...
  if (v06BackendState === 'not-configured') return 'Нужен ключ/voice ID';
...
window.renderAbout = function() {
  openPanel('О версии','<div class="item"><div class="item-title">Пятница v0.6-voice</div><div>Нейросетевой голос ElevenLabs через защищённый backend с автоматическим fallback на системный TTS.</div><div class="item-meta">ElevenLabs • backend proxy • системный fallback • отдельный профиль Вторника</div></div>',false);
};
...
v06Style.textContent = `
  .voice-url-input{width:100%;margin:10px 0 4px;border:1px solid rgba(126,228,255,.17);border-radius:15px;padding:12px 13px;background:rgba(255,255,255,.045);color:var(--text);font:inherit;outline:none;-webkit-user-select:text;user-select:text}
  .voice-url-input:focus{border-color:rgba(126,228,255,.5);box-shadow:0 0 0 1px rgba(65,199,255,.08) inset}
`;
